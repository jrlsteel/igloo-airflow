...
class ZendeskUserTickets:
    ...
    def extract_zendesk_user_tickets_json(self, data, account_id, k, dir_s3):

        df_zendesk_user_tickets = json_normalize(data)
        df_zendesk_user_tickets.columns = df_zendesk_user_tickets.columns.str.replace('.', '_')
        df_zendesk_user_tickets['account_id'] = account_id
        filename_zendesk_user_tickets = 'zendesk_user_tickets_' + str(account_id) + '.csv'
        df_zendesk_user_tickets_string = df_zendesk_user_tickets.to_csv(None, index=False)
        # print(df_account_transactions_string)

        k.key = dir_s3['s3_key']['ZendeskUserTickets'] + filename_account_transactions
    ...
    def log_error(self, error_msg, error_code=''):
        ...
        with open(sys.path[0] + '/logs/' + 'account_transactions_logs_' + time.strftime('%d%m%Y') + '.csv', mode='a') as errorlog:
