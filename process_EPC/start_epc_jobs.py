...
class StartEPCJobs:
    ...
    def submit_process_s3_mirror_job(self, source_input, destination_input):
        ...
        try:
            ...
            util.batch_logging_insert(
                self.mirror_jobid, 28, "epc_extract_mirror" + source_input + "-" + self.env, "start_epc_jobs.py"
            )
            ...
            print(
                "{0}: Process EPC files completed in {1:.2f} seconds".format(
    ...
    def submit_process_epc_certificates_job(self):
        ...
        try:
            util.batch_logging_insert(
                self.certificates_jobid, 16, "epc_certificates_extract_pyscript", "start_epc_jobs.py"
            )
            start = timeit.default_timer()
    ...
    def submit_process_epc_recommendations_job(self):
        ...
        try:
            util.batch_logging_insert(
                self.recommendations_jobid, 17, "epc_recommendations_extract_pyscript", "start_epc_jobs.py"
            )
            start = timeit.default_timer()
            ...
            print(
                "{0}: Process EPC Recommendations files completed in {1:.2f} seconds".format(
                    datetime.now().strftime("%H:%M:%S"), float(timeit.default_timer() - start)
                )
            )
        except Exception as e:
    ...
    def submit_epc_certificates_staging_gluejob(self):
        ...
        try:
            util.batch_logging_insert(
                self.certificates_staging_jobid, 18, "epc_certificates_staging_glue_job", "start_epc_jobs.py"
            )
            jobName = self.dir["glue_staging_job_name"]
            s3_bucket = self.dir["s3_bucket"]
            environment = self.env
            obj_stage = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="epc-certificates"
            )
    ...
    def submit_epc_recommendations_staging_gluejob(self):
        ...
        try:
            util.batch_logging_insert(
                self.recommendations_staging_jobid, 19, "epc_recommendations_staging_glue_job", "start_epc_jobs.py"
            )
            jobName = self.dir["glue_staging_job_name"]
            s3_bucket = self.dir["s3_bucket"]
            environment = self.env
            ...
            obj_stage = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="epc-recommendations"
            )
            staging_job_response = obj_stage.run_glue_job()
            if staging_job_response:
    ...
    def submit_ref_epc_certificates_gluejob(self):
        try:
            ...
            environment = self.env
            util.batch_logging_insert(
                self.certificates_ref_jobid, 20, "epc_certificates_ref_glue_job", "start_epc_jobs.py"
            )
            obj = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="epc_certificates"
            )
...
if __name__ == "__main__":
    ...
    if s.env == "prod":
        ...
        print(
            "EPC Certificates  Mirror  job is running...".format(
                datetime.now().strftime("%H:%M:%S"), s.process_epc_mirror_name
            )
        )
