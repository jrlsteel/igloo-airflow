...
class IglooEPCRecommendations:
    ...
    def log_error(self, error_msg, error_code=""):
        ...
        with open(
            sys.path[0] + "/logs/" + "igloo_epc_recommendations_log_" + time.strftime("%d%m%Y") + ".csv", mode="a"
        ) as errorlog:
    ...
    for i in range(n + 1):
        ...
        if i == n:
            t = multiprocessing.Process(
                target=p1.processAccounts, args=(lmkkey_sectors[lv:], s3_con(bucket_name), dir_s3)
            )
        else:
            t = multiprocessing.Process(
                target=p1.processAccounts, args=(lmkkey_sectors[lv:uv], s3_con(bucket_name), dir_s3)
            )
        lv = uv
