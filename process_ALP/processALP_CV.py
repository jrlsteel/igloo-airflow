...
class ALPHistoricalCV:
    ...
    def get_api_response(self, api_url, _end_date, _start_date, cv_folder):
        """
        ...
        body = """<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:mipi="http://www.NationalGrid.com/MIPI/">
                    ...
                    </soap:Envelope>""" % (_end_date, _start_date, cv_folder)
    ...
    def extract_cv_data(self, data_cv, cv_folder, k, dir_s3, start_date, end_date):
        ns = {'d': 'http://www.NationalGrid.com/MIPI/'}
        data = data_cv.xpath('//d:CLSMIPIPublicationObjectBE', namespaces=ns)
        ...
        #
            if alp_cv_df.empty:
                ...
                alp_cv_df += pandas.DataFrame([[name, applicable_at, applicable_for, value]],
                ...
                column_list = util.get_common_info('alp_column_order', 'cv')

                alp_cv_df_string = alp_cv_df.to_csv(None, columns=column_list, index=False)
    ...
    def log_error(self, error_msg, error_code=''):
        ...
        with open(logs_dir_path + 'historical_cv_log' + time.strftime('%d%m%Y') + '.csv',
                  mode='a') as errorlog:
            employee_writer = csv.writer(errorlog, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            employee_writer.writerow([error_msg, error_code])

    def processData(self, cv_folders, k, _dir_s3):
        for cv_folder in cv_folders:
            ...
            while _start_date < self.end_date:
                ...
                if api_response:
                    ...
                    self.extract_cv_data(formatted_xml, cv_folder, k, _dir_s3, _start_date, _end_date)
...
if __name__ == "__main__":
    ...
    cv_folders = con.test_config['CVFolders']
    '''Enable this to test for 1 account id'''
    #p.processData(cv_folders, s3, dir_s3)
    ...
    for i in range(n + 1):
        ...
        uv = i * k
        if i == n:
            t = multiprocessing.Process(target=p1.processData, args=(cv_folders[lv:], s3_con(bucket_name), dir_s3))
        else:
            t = multiprocessing.Process(target=p1.processData, args=(cv_folders[lv:uv], s3_con(bucket_name), dir_s3))
        lv = uv
