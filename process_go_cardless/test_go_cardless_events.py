...
from go_cardless_events import GoCardlessEventProcessor
...
def test_process_events_without_comma_in_description(mocker):
    ...
    mocker.patch.object(gc_event_processor.events_api,
                        'all', return_value=mock_events)
    ...
    assert(raw_data == '"EV01EB6FS11A8P","2020-11-12T11:07:47.778Z","payments","confirmed","","payment_confirmed","Enough time has passed since the payment was submitted for the banks to return an error so this payment is now confirmed.","gocardless","","","","","","","","","PM009X5MRFVPMT","","","",""')
    assert(column_headers == ['id', 'created_at', 'resource_type', 'action', 'customer_notifications', 'cause', 'description', 'origin', 'reason_code', 'scheme', 'will_attempt_retry', 'mandate', 'new_customer_bank_account', 'new_mandate', 'organisation', 'parent_event', 'payment', 'payout', 'previous_customer_bank_account', 'refund', 'subscription'])
    assert(list(reader) == [
