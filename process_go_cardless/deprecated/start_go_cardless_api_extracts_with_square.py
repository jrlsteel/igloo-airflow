...
class GoCardlessAPIExtracts:
    ...
    def extract_go_cardless_payments_job(self):
        ...
        try:
            util.batch_logging_insert(
                self.goCardless_jobid, 400, "go_cardless_payments.py", "start_go_cardless_api_extracts.py"
            )
            start = timeit.default_timer()
            ##subprocess.run([self.pythonAlias, "go_cardless_payments.py"], check=True)
            self.retry_function(
                process=subprocess.run([self.pythonAlias, "go_cardless_payments.py"], check=True),
                process_name="Go-Cardless Payments",
            )
            util.batch_logging_update(self.goCardless_jobid, "e")
            print(
                "{0}: Process Go-Cardless Payments API extract completed in {1:.2f} seconds".format(
    ...
    def extract_go_cardless_refunds_job(self):
        ...
        try:
            util.batch_logging_insert(
                self.goCardless_jobid, 400, "go_cardless_refunds.py", "start_go_cardless_api_extracts.py"
            )
            start = timeit.default_timer()
            ## subprocess.run([self.pythonAlias, "go_cardless_refunds.py"], check=True)
            self.retry_function(
                process=subprocess.run([self.pythonAlias, "go_cardless_refunds.py"], check=True),
                process_name="Go-Cardless Refunds",
            )
            util.batch_logging_update(self.goCardless_jobid, "e")
            print(
                "{0}: Process Go-Cardless Refunds API extract completed in {1:.2f} seconds".format(
                    datetime.now().strftime("%H:%M:%S"), float(timeit.default_timer() - start)
                )
            )
        except Exception as e:
    ...
    def extract_go_cardless_mandates_job(self):
        ...
        try:
            util.batch_logging_insert(
                self.goCardless_jobid, 400, "go_cardless_mandates.py", "start_go_cardless_api_extracts.py"
            )
            start = timeit.default_timer()
            ## subprocess.run([self.pythonAlias, "go_cardless_mandates.py"], check=True)
            self.retry_function(
                process=subprocess.run([self.pythonAlias, "go_cardless_mandates.py"], check=True),
                process_name="Go-Cardless Mandates",
            )
            util.batch_logging_update(self.goCardless_jobid, "e")
            print(
                "{0}: Process Go-Cardless Mandates API extract completed in {1:.2f} seconds".format(
                    datetime.now().strftime("%H:%M:%S"), float(timeit.default_timer() - start)
                )
            )
        except Exception as e:
    ...
    def extract_go_cardless_payouts_job(self):
        ...
        try:
            util.batch_logging_insert(
                self.goCardless_jobid, 400, "go_cardless_payout.py", "start_go_cardless_api_extracts.py"
            )
            start = timeit.default_timer()
            ## subprocess.run([self.pythonAlias, "go_cardless_payout.py"], check=True)
            self.retry_function(
                process=subprocess.run([self.pythonAlias, "go_cardless_payout.py"], check=True),
                process_name="Go-Cardless Payouts",
            )
            util.batch_logging_update(self.goCardless_jobid, "e")
            print(
                "{0}: Process Go-Cardless Payouts API extract completed in {1:.2f} seconds".format(
                    datetime.now().strftime("%H:%M:%S"), float(timeit.default_timer() - start)
                )
            )
        except Exception as e:
    ...
    def extract_go_cardless_events_job(self):
        ...
        try:
            util.batch_logging_insert(
                self.goCardless_jobid, 400, "go_cardless_events.py", "start_go_cardless_api_extracts.py"
            )
            start = timeit.default_timer()
            ## subprocess.run([self.pythonAlias, "go_cardless_events.py"], check=True)
            self.retry_function(
                process=subprocess.run([self.pythonAlias, "go_cardless_events.py"], check=True),
                process_name="Go-Cardless Events",
            )
            util.batch_logging_update(self.goCardless_jobid, "e")
            print(
                "{0}: Process Go-Cardless Events API extract completed in {1:.2f} seconds".format(
                    datetime.now().strftime("%H:%M:%S"), float(timeit.default_timer() - start)
                )
            )
        except Exception as e:
    ...
    def extract_go_cardless_customers_job(self):
        ...
        try:
            util.batch_logging_insert(
                self.goCardless_jobid, 400, "go_cardless_customers.py", "start_go_cardless_api_extracts.py"
            )
            start = timeit.default_timer()
            ## subprocess.run([self.pythonAlias, "go_cardless_customers.py"], check=True)
            self.retry_function(
                process=subprocess.run([self.pythonAlias, "go_cardless_customers.py"], check=True),
                process_name="Go-Cardless Customers",
            )
            util.batch_logging_update(self.goCardless_jobid, "e")
            print(
                "{0}: Process Go-Cardless Clients API extract completed in {1:.2f} seconds".format(
                    datetime.now().strftime("%H:%M:%S"), float(timeit.default_timer() - start)
                )
            )
        except Exception as e:
    ...
    def extract_go_cardless_subscriptions_job(self):
        ...
        print(
            "{0}: >>>> Process Go-Cardless Subscriptions API extract  <<<<".format(datetime.now().strftime("%H:%M:%S"))
        )
        try:
            util.batch_logging_insert(
                self.goCardless_jobid, 400, "go_cardless_subscriptions.py", "start_go_cardless_api_extracts.py"
            )
            start = timeit.default_timer()
            ## subprocess.run([self.pythonAlias, "go_cardless_subscriptions.py"], check=True)
            self.retry_function(
                process=subprocess.run([self.pythonAlias, "go_cardless_subscriptions.py"], check=True),
                process_name="Go-Cardless Subscriptions",
            )
            util.batch_logging_update(self.goCardless_jobid, "e")
            print(
                "{0}: Process Go-Cardless Clients API extract completed in {1:.2f} seconds".format(
    ...
    def extract_square_payments_job(self):
        ...
        try:
            util.batch_logging_insert(
                self.square_jobid, 401, "process_square_payments.py", "start_go_cardless_api_extracts.py"
            )
            start = timeit.default_timer()
            ## subprocess.run([self.pythonAlias, "../process_square/process_square_payments.py"], check=True)
            self.retry_function_square(
                process=subprocess.run([self.pythonAlias, "../process_square/process_square_payments.py"], check=True),
                process_name="Square Payments",
            )
    ...
    def submit_go_cardless_staging_gluejob(self):
        ...
        try:
            ...
            util.batch_logging_insert(
                self.staging_jobid, 403, "GoCardless staging gluejob", "start_go_cardless_api_extracts.py"
            )
    ...
    def submit_go_cardless_reporting_gluejob(self):
        ...
        try:
            ...
            environment = self.env
            util.batch_logging_insert(
                self.ref_jobid, 404, "GoCardless ref gluejob", "start_go_cardless_api_extracts.py"
            )
            obj_stage = glue.ProcessGlueJob(
