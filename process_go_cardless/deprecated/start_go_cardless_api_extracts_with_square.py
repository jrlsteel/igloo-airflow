...
class GoCardlessAPIExtracts:
    ...
    def extract_go_cardless_payments_job(self):
        ...
        try:
            util.batch_logging_insert(self.goCardless_jobid, 400, 'go_cardless_payments.py',
                                      'start_go_cardless_api_extracts.py')
            start = timeit.default_timer()
            ##subprocess.run([self.pythonAlias, "go_cardless_payments.py"], check=True)
            self.retry_function(process=subprocess.run([self.pythonAlias, "go_cardless_payments.py"], check=True),
                                process_name='Go-Cardless Payments')
            util.batch_logging_update(self.goCardless_jobid, 'e')
            print("{0}: Process Go-Cardless Payments API extract completed in {1:.2f} seconds".format(
    ...
    def extract_go_cardless_refunds_job(self):
        ...
        try:
            ...
            self.retry_function(process=subprocess.run([self.pythonAlias, "go_cardless_refunds.py"], check=True),
                                process_name='Go-Cardless Refunds')
            util.batch_logging_update(self.goCardless_jobid, 'e')
            print("{0}: Process Go-Cardless Refunds API extract completed in {1:.2f} seconds".format(
                datetime.now().strftime('%H:%M:%S'),
                float(timeit.default_timer() - start)))
        except Exception as e:
            util.batch_logging_update(self.goCardless_jobid, 'f', str(e))
    ...
    def extract_go_cardless_mandates_job(self):
        ...
        try:
            ...
            self.retry_function(process=subprocess.run([self.pythonAlias, "go_cardless_mandates.py"], check=True),
                                process_name='Go-Cardless Mandates')
            util.batch_logging_update(self.goCardless_jobid, 'e')
            print("{0}: Process Go-Cardless Mandates API extract completed in {1:.2f} seconds".format(
                datetime.now().strftime('%H:%M:%S'),
                float(timeit.default_timer() - start)))
        except Exception as e:
            util.batch_logging_update(self.goCardless_jobid, 'f', str(e))
    ...
    def extract_go_cardless_subscriptions_job(self):
        ...
        try:
            ...
            self.retry_function(process=subprocess.run([self.pythonAlias, "go_cardless_subscriptions.py"], check=True),
    ...
    def extract_square_payments_job(self):
        ...
        try:
            ...
            print("{0}: Process Square Payments API extract completed in {1:.2f} seconds".format(
                datetime.now().strftime('%H:%M:%S'),
                float(timeit.default_timer() - start)))
        except Exception as e:
            util.batch_logging_update(self.square_jobid, 'f', str(e))
    ...
    def submit_go_cardless_staging_gluejob(self):
        ...
        try:
            ...
            util.batch_logging_insert(self.staging_jobid, 403, 'GoCardless staging gluejob', 'start_go_cardless_api_extracts.py')
    ...
    def submit_go_cardless_reporting_gluejob(self):
        ...
        try:
            ...
            s3_bucket = self.dir['s3_bucket']
            environment = self.env
            util.batch_logging_insert(self.ref_jobid, 404, 'GoCardless ref gluejob', 'start_go_cardless_api_extracts.py')
            obj_stage = glue.ProcessGlueJob(job_name=jobName, s3_bucket=s3_bucket, environment=environment,
