...
class EstimatedAdvance:
    ...
    def submit_estimated_advance_gluejob(self):
        try:
            ...
            # Batch Logging
            util.batch_logging_insert(self.est_advance_job_id, 50, 'estimated_advance_glue_job', 'start_est_advance_job.py')

            obj_est_adv = glue.ProcessGlueJob(job_name=self.jobName, s3_bucket=self.s3_bucket, environment=self.environment, processJob='est_adv')
            response = obj_est_adv.run_glue_job()

            if response:

                print("{0}: Estimated {1} Completed successfully".format(datetime.now().strftime('%H:%M:%S'), self.jobName))
