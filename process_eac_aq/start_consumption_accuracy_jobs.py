...
class ConsumptionAccuracy:
    ...
    def submit_consumption_accuracy_gluejob(self):
        try:
            util.batch_logging_insert(self.cons_accu_ref_jobid, 42, 'eac_cons_accu_glue_job', 'start_consumption_accuracy_jobs.py')

            jobName = self.dir['glue_cons_accu_job_name']
            s3_bucket = self.dir['s3_bucket']
            environment = self.env

            obj_eac_aq = glue.ProcessGlueJob(job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob='cons_accu')
