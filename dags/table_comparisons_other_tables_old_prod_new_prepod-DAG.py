...
dag = DAG(
    dag_id='table_comparisons_other_tables_old_prod_new_preprod',
    default_args=args,
...
for task_id in task_ids:
    operator = BashOperator(
        task_id=task_id,
        bash_command='cd /opt/airflow/enzek-meterpoint-readings/process_table_comparisons && python compare_tables.py --table-comparison-config old_prod_new_preprod_{} --output-to-s3'.format(task_id),
        dag=dag
    )
    d1 >> operator
