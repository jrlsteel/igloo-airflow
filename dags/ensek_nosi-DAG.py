...
mirror_nosi = BashOperator(
    task_id="mirror_nosi",
    bash_command="cd /opt/airflow/cdw/process_Nosi && python start_ensek_readings_nosi_mirror_only_jobs.py",
    dag=dag,
)

start_ensek_readings_nosi_staging_jobs = BashOperator(
    task_id="start_ensek_readings_nosi_staging_jobs",
    bash_command="cd /opt/airflow/cdw/process_Nosi && python start_ensek_readings_nosi_staging_jobs.py",
    dag=dag,
)

start_ensek_readings_nosi_ref_jobs = BashOperator(
    task_id="start_ensek_readings_nosi_ref_jobs",
    bash_command="cd /opt/airflow/cdw/process_Nosi && python start_ensek_readings_nosi_ref_jobs.py",
    dag=dag,
)
