...
process_land_registry = BashOperator(
    ...
    bash_command="cd /opt/airflow/cdw/process_LandRegistry && python process_land_registry.py",
    dag=dag,
)

start_land_registry_staging_jobs = BashOperator(
    task_id="start_land_registry_staging_jobs",
    bash_command="cd /opt/airflow/cdw/process_LandRegistry && python start_land_registry_staging_jobs.py",
    dag=dag,
)

start_land_registry_ref_jobs = BashOperator(
    task_id="start_land_registry_ref_jobs",
    bash_command="cd /opt/airflow/cdw/process_LandRegistry && python start_land_registry_ref_jobs.py",
    dag=dag,
)
