...
from cdw.common.utils import IglooLogger
from cdw.common.etl import ETLPipelineWrapper, ETLPipelineWrapperRuntimeException
    ...
    def run_postcodes_api_extract(self):
        ...
        try:
            util.batch_logging_insert(
                self.postcodes_api_extract_job_id,
                self.job_id,
                self.postcodes_api_extract_job_name,
                os.path.basename(__file__),
            )
            start = timeit.default_timer()
            subprocess.run([self.pythonAlias, "postcodes_etl.py"], check=True)
            self.logger.in_prod_env(
                "{} completed in {:.2f} seconds".format(
                    self.postcodes_api_extract_job_name, float(timeit.default_timer() - start)
                )
            )
