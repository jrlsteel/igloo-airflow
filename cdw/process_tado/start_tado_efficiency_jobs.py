    ...
    def submit_tado_efficiency_batch_gluejob(self):
        try:
            ...
            util.batch_logging_insert(
                self.tado_efficiency_batch_jobid, 1, "tado_efficiency_gluejob", "start_tado_efficiency_jobs.py"
            )

            obj_tado_efficiency = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="tado_efficiency"
            )
