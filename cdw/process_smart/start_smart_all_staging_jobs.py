    ...
    def submit_smart_all_staging_gluejob(self):
        try:
            util.batch_logging_insert(
                self.smart_all_staging_jobid, 601, "smart_all_staging_glue_job", "start_smart_all_staging_jobs.py"
            )
            ...
            obj_stage = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="smart-all"
            )
