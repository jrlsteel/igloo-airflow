...
import datetime
from cdw.connections.connect_db import get_boto_S3_Connections
...
def fetch_d0379_accounts():
    ...
    d0379_accounts_sql_query = cdw.common.directories.common["elective_hh"]["elective_hh_trial_participants_sql"]

    logger.info("Exeuting SQL: {}".format(d0379_accounts_sql_query))

    d0379_accounts_df = cdw.common.utils.execute_query_return_df(d0379_accounts_sql_query)
...
def fetch_d0379_data(d0379_accounts, d0379_date):
    ...
    sql_query = cdw.common.directories.common["elective_hh"]["sql_query"].format(
    ...
    df = cdw.common.utils.execute_query_return_df(sql_query)
...
def generate_d0379(d0379_date):
    ...
    try:
        ...
        s3_key_prefix = cdw.common.directories.common["elective_hh"]["d0379_s3_key_prefix"]
...
def copy_d0379_to_sftp(d0379_date):
    ...
    try:
        ...
        s3_key_prefix = "{}/D0379_{}_".format(
            cdw.common.directories.common["elective_hh"]["d0379_s3_key_prefix"],
            d0379_date.strftime("%Y%m%d"),
        )
