...
from cdw.process_smart.process_usmart_asei_smart_reads_billing import SmartReadsBillings
...
def mock_airflow_variable(key_name):
    print("Mock function for variable received keyname of {}".format(key_name))
    if key_name == "ENVIRONMENT":
        return "uat"
    else:
        raise Exception("mock received a value that was not expected")


class TestSmartReadsBillings(unittest.TestCase):

    # Patch in our mocks for the util functions
    @patch("cdw.common.utils.get_api_info", mock_util_get_api_info)
    @patch("cdw.common.utils.get_smart_read_billing_api_info", mock_util_get_smart_read_billing_api_info)
    @patch(
        "cdw.process_smart.process_usmart_asei_smart_reads_billing.SmartReadsBillings.get_env", mock_airflow_variable
    )
