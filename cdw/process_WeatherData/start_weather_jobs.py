...
class Weather:
    ...
    def submit_staging_gluejob(self):
        try:
            util.batch_logging_insert(
                self.weather_staging_jobid, 22, "weather_staging_glue_job", "start_weather_jobs.py"
            )
            jobName = self.dir["glue_staging_job_name"]
            s3_bucket = self.dir["s3_bucket"]
            environment = self.env
            obj_stage = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="historicalweather"
            )
