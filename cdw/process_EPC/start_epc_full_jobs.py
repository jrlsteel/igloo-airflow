...
class StartEPCJobs:
    ...
    def submit_process_s3_mirror_job(self, source_input, destination_input):
        ...
        try:
            ...
            util.batch_logging_insert(
                self.mirror_jobid,
                36,
                "epc_full_extract_mirror" + source_input + "-" + self.env,
            ...
            print(
                "{0}: Process EPC Full files completed in {1:.2f} seconds".format(
    ...
    def submit_epc_certificates_staging_gluejob(self):
        ...
        try:
            util.batch_logging_insert(
                self.certificates_staging_jobid, 57, "epc_certificates_staging_glue_job", "start_epc_full_jobs.py"
            )
            jobName = self.dir["glue_staging_job_name"]
            s3_bucket = self.dir["s3_bucket"]
            environment = self.env
            obj_stage = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="epc-certificates-all"
            )
    ...
    def submit_epc_recommendations_staging_gluejob(self):
        ...
        try:
            util.batch_logging_insert(
                self.recommendations_staging_jobid, 58, "epc_recommendations_staging_glue_job", "start_epc_full_jobs.py"
            )
            jobName = self.dir["glue_staging_job_name"]
            s3_bucket = self.dir["s3_bucket"]
            environment = self.env
            ...
            obj_stage = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="epc-recommendations-all"
            )
            staging_job_response = obj_stage.run_glue_job()
            if staging_job_response:
