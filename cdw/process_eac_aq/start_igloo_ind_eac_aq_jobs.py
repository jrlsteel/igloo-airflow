    ...
    def submit_eac_aq_gluejob(self):
        try:
            util.batch_logging_insert(
                self.igl_ind_eac_aq_ref_jobid, 40, "igl_ind_eac_aq_calculated_glue_job", "start_igl_ind_eac_aq_jobs.py"
            )
            ...
            obj_eac_aq = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="igl_ind_eac_aq"
            )
            ...
            if eac_aq_job_response:
                ...
                print(
                    "{0}: Igloo Ind EAC and AQ Job Completed successfully".format(datetime.now().strftime("%H:%M:%S"))
                )
