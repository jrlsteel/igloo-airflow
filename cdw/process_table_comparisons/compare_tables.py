...
from cdw.common.utils import batch_logging_insert, batch_logging_update, get_jobID
...
def compare_calculated_tables(stage=None):
    ...
    try:
        ...
        if stage is None:
            from cdw.conf.config import environment_config as current_environment
...
def compare_tables(table_comparison_config_name, output_to_s3=False):
    ...
    try:
        from cdw.conf.config import table_comparison_configs
        ...
        if output_to_s3:
            from cdw.conf.config import s3_config
