...
class StartInternalEstimatesJobs:
    ...
    def submit_internal_estimates_job(self):
        ...
        try:
            ...
            subprocess.run([self.pythonAlias, "process_ensek_internal_estimates.py"], check=True)
            print(
                "{0}: Process Ensek Internal Estimates completed in {1:.2f} seconds".format(
    ...
    def submit_internal_estimates_staging_gluejob(self):
        try:
            ...
            obj_stage = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="ensek-internal-estimates"
            )
            job_response = obj_stage.run_glue_job()
            if job_response:
                print(
                    "{0}: Staging Ensek Internal Estimates Job Completed successfully".format(
    ...
    def submit_internal_estimates_gluejob(self):
        try:
            ...
            obj_submit_internal_estimates_Gluejob = glue.ProcessGlueJob(
                job_name=jobname, s3_bucket=s3_bucket, environment=environment, processJob="ensek_ref_estimates"
            )
            job_response = obj_submit_internal_estimates_Gluejob.run_glue_job()
            if job_response:
                print(
                    "{0}: Ensek Internal Estimates Glue Job completed successfully".format(
                        datetime.now().strftime("%H:%M:%S")
                    )
                )
