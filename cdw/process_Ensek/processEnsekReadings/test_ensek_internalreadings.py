...
from cdw.process_Ensek.processEnsekReadings.process_ensek_internal_readings import InternalReadings
...
def test_extract_data():
    ...
    # Get the test data
    data_file = open(os.path.join(os.path.dirname(__file__), "fixtures", "response-1831-page1.json"))
    ...
    internalreadings.extract_internal_data_response(data_json, account_id, bucket_name, dir_s3, metrics)
    ...
    assert internal_readings_column_headers == internal_readings_header_s3, "failed internal readings headers"

    assert list(internalreadings_reader) == [
        [
            "1831",
            "True",
            "True",
        ...
        [],
    ], "failed internal_readings data content"
...
def test_api_response_one_page():
    ...
    responses.add(responses.POST, "{}".format(url), body=data_json_1, status=200)
...
def test_api_response_multiple_pages():
    ...
    responses.add(responses.POST, "{}/{}?page=2".format(url, account_id), body=data_json_1, status=200)
    responses.add(responses.POST, "{}/{}?page=3".format(url, account_id), body=data_json_2, status=200)
    responses.add(responses.POST, "{}/{}?page=4".format(url, account_id), body=data_json_3, status=200)
    responses.add(responses.POST, "{}/{}".format(url, account_id), body=data_json_0, status=200)

    response = internalreadings.get_api_response("{}/{}".format(url, account_id), head, account_id, metrics)

    complete_items = (
        json.loads(data_json_0)["items"]
