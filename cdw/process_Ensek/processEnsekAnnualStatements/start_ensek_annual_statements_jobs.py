...
class StartAnnualStatementsJobs:
    ...
    def submit_annual_statements_job(self):
        ...
        try:
            ...
            subprocess.run([self.pythonAlias, "process_ensek_annual_statements.py"], check=True)
            print(
                "{0}: Process Ensek Annual Statements completed in {1:.2f} seconds".format(
    ...
    def submit_annual_statements_staging_gluejob(self):
        try:
            ...
            obj_stage = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="ensek-annual-statements"
            )
            job_response = obj_stage.run_glue_job()
            if job_response:
                print(
                    "{0}: Staging Ensek Annual Statements Job Completed successfully".format(
    ...
    def submit_annual_statements_gluejob(self):
        try:
            ...
            obj_submit_internal_estimates_Gluejob = glue.ProcessGlueJob(
                job_name=jobname, s3_bucket=s3_bucket, environment=environment, processJob="ensek_ref_annual_statements"
            )
            job_response = obj_submit_internal_estimates_Gluejob.run_glue_job()
            if job_response:
                print(
                    "{0}: Ensek Annual Statements Glue Job completed successfully".format(
                        datetime.now().strftime("%H:%M:%S")
                    )
                )
