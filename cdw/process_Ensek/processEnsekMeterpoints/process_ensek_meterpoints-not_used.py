...
class MeterPoints:
    ...
    def extract_meter_point_json(self, data, account_id, k, dir_s3):
        ...
        if df_meterpoints.empty:
            ...
            file_name_meterpoints = "meter_points_history_" + str(account_id) + ".csv"
            # k.key = 'ensek-meterpoints/MeterPoints/' + file_name_meterpoints
            k.key = dir_s3["s3_key"]["MeterPointsHistory"] + file_name_meterpoints
        ...
        if df_meters.empty:
            ...
            filename_meters = "meters_history_" + str(account_id) + ".csv"
            # k.key = 'ensek-meterpoints/Meters/' + filename_meters
            k.key = dir_s3["s3_key"]["MetersHistory"] + filename_meters
        ...
        if df_attributes.empty:
            ...
            k.key = dir_s3["s3_key"]["MeterPointsAttributesHistory"] + filename_attributes
        ...
        if df_registers.empty:
            ...
            df_registers1.rename(
                columns={"meter_point_meters_meterId": "meter_id", "registers_id": "register_id"}, inplace=True
            )
            ...
            filename_registers = "registers_history_" + str(account_id) + ".csv"
            # k.key = 'ensek-meterpoints/Registers/' + filename_registers
            k.key = dir_s3["s3_key"]["RegistersHistory"] + filename_registers
        ...
        if df_registersAttributes.empty:
            ...
            filename_registersAttributes = "registersAttributes_history_" + str(account_id) + ".csv"
            # k.key = 'ensek-meterpoints/RegistersAttributes/' + filename_registersAttributes
            k.key = dir_s3["s3_key"]["RegistersAttributesHistory"] + filename_registersAttributes
        ...
        if df_metersAttributes.empty:
            ...
            filename_metersAttributes = "metersAttributes_history_" + str(account_id) + ".csv"
            # k.key = 'ensek-meterpoints/MetersAttributes/' + filename_metersAttributes
            k.key = dir_s3["s3_key"]["MetersAttributesHistory"] + filename_metersAttributes
    ...
    def extract_meter_readings_json(self, data, account_id, meter_point_id, k, _dir_s3):
        ...
        filename_readings = "mp_readings_history_" + str(account_id) + "_" + str(meter_point_id) + ".csv"
        ...
        k.key = _dir_s3["s3_key"]["ReadingsHistory"] + filename_readings
    ...
    def extract_meter_readings_billeable_json(self, data, account_id, meter_point_id, k, _dir_s3):
        ...
        df_meter_readings["meter_point_id"] = meter_point_id
        # df_meter_readings.to_csv('meter_point_readings_' + str(account_id) + '_' + str(meter_point_id) + '_' + '.csv')
        filename_readings = "mp_readings_billeable_history_" + str(account_id) + "_" + str(meter_point_id) + ".csv"
        df_meter_readings_string = df_meter_readings.to_csv(None, index=False)
        # k.key = 'ensek-meterpoints/ReadingsBilleable/' + filename_readings
        k.key = _dir_s3["s3_key"]["ReadingsBilleableHistory"] + filename_readings
        k.set_contents_from_string(df_meter_readings_string)
        # print(df_meter_readings_string)
        # print(filename_readings)
