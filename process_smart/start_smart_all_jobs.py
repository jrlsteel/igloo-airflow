...
class SmartReadingProcessingJobs:
    ...
    def submit_start_smart_all_reporting_jobs_job(self):
        ...
        try:
            util.batch_logging_insert(self.start_smart_all_reporting_jobs_jobid, 51, 'start_smart_all_reporting_jobs',
            ...
            print("{0}: start_smart_all_reporting_jobs  completed in {1:.2f} seconds".format(datetime.now().strftime('%H:%M:%S'),
    ...
    def submit_process_smart_reads_billing_job(self):
        ...
        try:
            util.batch_logging_insert(self.start_smart_all_billing_jobs_jobid, 51, 'Smart Reads Billing pyscript',
                                      'process_smart_reads_billing.py')
            start = timeit.default_timer()
            subprocess.run([self.pythonAlias, "process_smart_reads_billing.py"], check=True)
            print("{0}: process_smart_reads_billing  completed in {1:.2f} seconds".format(datetime.now().strftime('%H:%M:%S'),
                                                                               float(timeit.default_timer() - start)))
...
if __name__ == '__main__':
    ...
    util.batch_logging_insert(s.start_smart_all_jobid, 51, 'Smart All Jobs',
                              'start_smart_all_jobs.py')
