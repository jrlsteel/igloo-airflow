        ...
        for index, df in _df.iterrows():
            with metrics["meterpointnumber_counter"].get_lock():
                ...
                if metrics["meterpointnumber_counter"].value % 1000 == 0:
                    iglog.in_prod_env(
                        "Meterpoints processesed: {}".format(
                            str(metrics["meterpointnumber_counter"].value)
                        )
                    )
        ...
        try:
            for i in range(1, total_processes + 1):
                ...
                if i == total_processes:
                    ...
                    t = multiprocessing.Process(target=p1.process_accounts, args=(df[lv:], connections.connect_db.get_boto_S3_Connections(bucket_name), dir_s3, metrics))
                else:
                    iglog.in_prod_env(f'Starting process {i} with {len(df[lv:uv])} items')
                    t = multiprocessing.Process(target=p1.process_accounts, args=(df[lv:uv], connections.connect_db.get_boto_S3_Connections(bucket_name), dir_s3, metrics))
                lv = uv
