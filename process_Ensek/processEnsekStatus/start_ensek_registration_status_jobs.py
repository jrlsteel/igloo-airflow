...
class StartRegistrationsMeterpointsStatusJobs:
    ...
    def submit_registrations_meterpoints_status_job(self):
        ...
        print(
            "{0}: >>>> Process Ensek Registrations by Meterpoint Status  <<<<".format(
                datetime.now().strftime("%H:%M:%S")
            )
        )
        try:
            start = timeit.default_timer()
            subprocess.run([self.pythonAlias, "process_ensek_registration_meterpoint_status.py"], check=True)
            print(
                "{0}: Process Ensek Registrations by Meterpoint Status completed in {1:.2f} seconds".format(
    ...
    def submit_registrations_meterpoints_status_staging_gluejob(self):
        try:
            ...
            obj_stage = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="reg-mp-status"
            )
    ...
    def submit_registrations_meterpoints_status_gluejob(self):
        try:
            ...
            obj_submit_registrations_meterpoints_status_Gluejob = glue.ProcessGlueJob(
                job_name=jobname, s3_bucket=s3_bucket, environment=environment, processJob="reg_mp_status"
            )
            job_response = obj_submit_registrations_meterpoints_status_Gluejob.run_glue_job()
            if job_response:
                print(
                    "{0}: Registrations Meterpoints Status Glue Job completed successfully".format(
                        datetime.now().strftime("%H:%M:%S")
                    )
                )
...
if __name__ == "__main__":
    ...
    print(
        "{0}: All Registrations Meterpoints Status completed successfully".format(datetime.now().strftime("%H:%M:%S"))
    )
