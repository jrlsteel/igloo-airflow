...
from process_ensek_registration_meterpoint_status import RegistrationsMeterpointsStatus, main
...
def test_process_accounts_0_accounts():
    # Arrange
    df = pandas.DataFrame.from_records([], columns=['account_id', 'meter_point_id', 'meterpointnumber', 'meterpointtype'])
...
@responses.activate
@unittest.mock.patch("process_ensek_registration_meterpoint_status.RegistrationsMeterpointsStatus.extract_reg_elec_json")
def test_process_accounts_1_elec_account(mock_extract_reg_elec_json):
    # Arrange
    df = pandas.DataFrame.from_records([(1831, 2102, 2000007794886, 'E')], columns=['account_id', 'meter_point_id', 'meterpointnumber', 'meterpointtype'])
    ensek_response_body = '"Tracker.Registration.Live"'
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/1831/Processes/Registrations/Elec?meterpointnumber=2000007794886', body=ensek_response_body)
...
@responses.activate
@unittest.mock.patch("process_ensek_registration_meterpoint_status.RegistrationsMeterpointsStatus.extract_reg_gas_json")
def test_process_accounts_1_gas_account(mock_extract_reg_gas_json):
    # Arrange
    df = pandas.DataFrame.from_records([(1831, 2102, 2000007794886, 'G')], columns=['account_id', 'meter_point_id', 'meterpointnumber', 'meterpointtype'])
    ensek_response_body = '"Tracker.Registration.Live"'
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/1831/Processes/Registrations/Gas?meterpointnumber=2000007794886', body=ensek_response_body)
...
def test_process_accounts_10_accounts(mock_extract_reg_gas_json, mock_extract_reg_elec_json):
    ...
    ensek_response_body = '"Tracker.Registration.Live-{}"'
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/1/Processes/Registrations/Gas?meterpointnumber=111', body=ensek_response_body.format(1))
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/2/Processes/Registrations/Elec?meterpointnumber=222', body=ensek_response_body.format(2))
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/3/Processes/Registrations/Gas?meterpointnumber=333', body=ensek_response_body.format(3))
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/4/Processes/Registrations/Elec?meterpointnumber=444', body=ensek_response_body.format(4))
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/5/Processes/Registrations/Gas?meterpointnumber=555', body=ensek_response_body.format(5))
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/6/Processes/Registrations/Elec?meterpointnumber=666', body=ensek_response_body.format(6))
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/7/Processes/Registrations/Gas?meterpointnumber=777', body=ensek_response_body.format(7))
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/8/Processes/Registrations/Elec?meterpointnumber=888', body=ensek_response_body.format(8))
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/9/Processes/Registrations/Gas?meterpointnumber=999', body=ensek_response_body.format(9))
    responses.add(responses.GET, 'https://api.igloo.ignition.ensek.co.uk/Accounts/10/Processes/Registrations/Elec?meterpointnumber=101010', body=ensek_response_body.format(10))
...
@unittest.mock.patch("common.utils.execute_query")
@unittest.mock.patch("multiprocessing.Process")
@unittest.mock.patch("connections.connect_db.get_boto_S3_Connections")
def test_main(mock_get_boto_S3_Connections, mock_multiprocessing_process, mock_execute_query):
    ...
    assert mock_multiprocessing_process.call_args_list[0][1]["args"][0].equals(pandas.DataFrame(
        [
            [ 8046, 408903, 9309500700, 'G' ],
            [ 8046, 13809, 1900034071970, 'E' ],
            [ 21112, 36271, 1300009258842, 'E' ],
    ...
    assert mock_multiprocessing_process.call_args_list[1][1]["args"][0].equals(pandas.DataFrame(
        [
            [ 46577, 411919, 1637435702, 'G' ],
            [ 63666, 101436, 3972193101, 'G' ],
            [ 73125, 357751, 2229158607, 'G' ],
    ...
    assert mock_multiprocessing_process.call_args_list[2][1]["args"][0].equals(pandas.DataFrame(
        [
            [ 79243, 437688, 2000050831720, 'E' ],
            [ 86073, 137615, 1610030856299, 'E' ],
            [ 106332, 171225, 1610004694967, 'E' ],
    ...
    assert mock_multiprocessing_process.call_args_list[3][1]["args"][0].equals(pandas.DataFrame(
        [
            [ 107047, 416124, 7656943003, 'G' ],
            [ 120445, 400785, 4256548707, 'G' ],
            [ 124796, 198680, 1012568781947, 'E' ],
    ...
    assert mock_multiprocessing_process.call_args_list[4][1]["args"][0].equals(pandas.DataFrame(
        [
            [ 132801, 208820, 3965995708, 'G' ],
            [ 136458, 215369, 1141606003, 'G' ],
            [ 141117, 223681, 1610011357349, 'E' ],
    ...
    assert mock_multiprocessing_process.call_args_list[5][1]["args"][0].equals(pandas.DataFrame(
        [
            [ 152207, 243409, 522554809, 'G' ],
            [ 169975, 271890, 1012438580093, 'E' ],
            [ 175838, 269256, 3387138003, 'G' ],
