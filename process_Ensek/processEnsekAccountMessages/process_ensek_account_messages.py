...
def extract_tariff_history_json(data, account_id, k, dir_s3):
    # global k,
    meta_tariff_history = ['tariffName', 'startDate', 'endDate', 'discounts', 'tariffType', 'exitFees', 'id', 'account_id']
...
def log_error(error_msg, error_code=''):
    ...
    if not os.path.exists(logs_dir_path):
        ...
        with open(sys.path[0] + '/logs/' + 'account_messages_logs_' + time.strftime('%d%m%Y') + '.csv',mode='a') as errorlog:
...
if __name__ == "__main__":
    ...
    p10 = multiprocessing.Process(target=processAccounts, args=(account_ids[9 * p:10 * p], s3_con(bucket_name), dir_s3))
    p11 = multiprocessing.Process(target=processAccounts, args=(account_ids[10 * p:11 * p], s3_con(bucket_name), dir_s3))
