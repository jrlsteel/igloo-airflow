    ...
    def submit_direct_debits_extract_staging_gluejob(self):
        try:
            ...
            obj_stage = glue.ProcessGlueJob(job_name=jobName, s3_bucket=s3_bucket, environment=environment,
                                            processJob='direct-debits')
            job_response = obj_stage.run_glue_job()
            if job_response:
                print("{0}: Staging Ensek Direct Debits Job Completed successfully".format(datetime.now().strftime('%H:%M:%S')))
    ...
    def submit_direct_debits_extract_reference_gluejob(self):
        try:
            ...
            obj_submit_registrations_meterpoints_status_Gluejob = glue.ProcessGlueJob(job_name=jobname, s3_bucket=s3_bucket, environment=environment,
                                                 processJob='ref_direct_debits')
            job_response = obj_submit_registrations_meterpoints_status_Gluejob.run_glue_job()
            if job_response:
                print("{0}: Ensek Direct Debits Reference Glue Job completed successfully".format(datetime.now().strftime('%H:%M:%S')))
            else:
