...
def test_extract_direct_debit_json():
    ...
    direct_debit.extract_direct_debit_json(
        data, account_id, k, dir_s3)

    expected_s3_key = 'stage1/DirectDebit/direct_debit{}.csv'.format(
        account_id)
    ...
    assert(column_headers == ['Amount', 'BankAccount', 'BankName',
...
def test_extract_direct_debit_health_check_json():
    ...
    direct_debit.extract_direct_debit_health_check_json(
        data, account_id, k, dir_s3)

    expected_s3_key = 'stage1/DirectDebitHealthCheck/dd_heath_check_{}.csv'.format(
        account_id)
    ...
    column_headers = next(reader)
    assert(column_headers == ['directDebitIsActive', 'isTopup', 'paymentAmount', 'paymentDay', 'DirectDebitType', 'BankAccountIsActive', 'DirectDebitStatus',
                              'NextAvailablePaymentDate', 'AccountName', 'Amount', 'BankName', 'BankAccount', 'PaymentDate', 'Reference', 'SortCode', 'account_id'])
    assert(list(reader) == [
        ['True', 'False', '98.0', '17', 'Fixed', 'True', 'Authorised', '2020-11-10T00:00:00',
            '', '98.0', 'BARCLAYS BANK PLC', '******55', '17', '', '', '1831'],
        []  # FIXME: no idea why there's an empty list at the end.
    ])
