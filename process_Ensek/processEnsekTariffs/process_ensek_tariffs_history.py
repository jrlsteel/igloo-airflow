...
class TariffHistory(object):
    ...
    def extract_tariff_history_json(self, data, account_id, k, dir_s3):
        ...
        # Elec
        if ("Electricity" in df_tariff_history.columns and not df_tariff_history["Electricity"].isnull) or not (
            "Electricity" in df_tariff_history.columns
        ):
            # UnitRates
            meta_elec_unitrates = ["tariffName", "startDate", "endDate"]
            df_elec_unit_rates = pandas.json_normalize(
                data, record_path=[["Electricity", "unitRates"]], meta=meta_elec_unitrates
            )
            ...
            df_elec_standing_charge = pandas.json_normalize(
                data, record_path=[["Electricity", "standingChargeRates"]], meta=meta_elec_standing_charge
            )
        ...
        # Gas
        if ("Gas" in df_tariff_history.columns and not df_tariff_history["Gas"].isnull) or not (
            "Gas" in df_tariff_history.columns
        ):
            # UnitRates
            ...
            df_elec_standing_charge = pandas.json_normalize(
                data, record_path=[["Gas", "standingChargeRates"]], meta=meta_gas_standing_charge
            )
        ...
        for account_id in account_ids:
            with metrics["account_id_counter"].get_lock():
                ...
                if metrics["account_id_counter"].value % 1000 == 0:
                    iglog.in_prod_env("Account IDs processesed: {}".format(str(metrics["account_id_counter"].value)))
            ...
            for i in range(1, total_processes + 1):
                ...
                if i == total_processes:
                    ...
                    t = multiprocessing.Process(
                        target=p1.process_accounts, args=(account_ids[lv:], s3_con(bucket_name), dir_s3, metrics)
                    )
                else:
                    iglog.in_prod_env(f"Starting process {i} with {len(account_ids[lv:uv])} accounts")
                    t = multiprocessing.Process(
                        target=p1.process_accounts, args=(account_ids[lv:uv], s3_con(bucket_name), dir_s3, metrics)
                    )
                lv = uv
