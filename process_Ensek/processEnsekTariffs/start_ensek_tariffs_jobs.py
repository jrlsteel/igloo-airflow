...
class StartTariffHistoryJobs:
    ...
    def submit_tariff_history_job(self):
        ...
        try:
            ...
            subprocess.run([self.pythonAlias, "process_ensek_tariff_history.py"])
            print("{0}: Process Ensek Tariff History completed in {1:.2f} seconds".format(datetime.now().strftime('%H:%M:%S'),
    ...
    def submit_tariff_history_staging_gluejob(self):
        try:
            ...
            obj_stage = glue.ProcessGlueJob(job_name=jobName, s3_bucket=s3_bucket, environment=environment,
                                            processJob='tariff-history')
            job_response = obj_stage.run_glue_job()
            if job_response:
                print("{0}: Tariff History Staging Job Completed successfully".format(datetime.now().strftime('%H:%M:%S')))
    ...
    def submit_tariff_history_gluejob(self):
        try:
            ...
            obj_submit_tariff_history_Gluejob = glue.ProcessGlueJob(job_name=jobname, s3_bucket=s3_bucket, environment=environment,
                                                 processJob='tariff_history')
