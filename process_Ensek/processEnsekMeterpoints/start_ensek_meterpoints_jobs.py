...
class StartMeterpointsJobs:
    ...
    def submit_meterpoints_extract_job(self):
        ...
        try:
            ...
            subprocess.run([self.pythonAlias, "process_ensek_meterpoints.py"], check=True)
            print(
                "{0}: Process Ensek Meterpoints completed in {1:.2f} seconds".format(
    ...
    def submit_meterpoints_extract_staging_gluejob(self):
        try:
            ...
            obj_stage = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="meterpoints"
            )
            job_response = obj_stage.run_glue_job()
            if job_response:
                print(
                    "{0}: Staging Ensek Meterpoints Job Completed successfully".format(
    ...
    def submit_meterpoints_extract_reference_gluejob(self):
        try:
            ...
            obj_submit_registrations_meterpoints_status_Gluejob = glue.ProcessGlueJob(
                job_name=jobname, s3_bucket=s3_bucket, environment=environment, processJob="ref_meterpoints"
            )
            job_response = obj_submit_registrations_meterpoints_status_Gluejob.run_glue_job()
            if job_response:
                print(
                    "{0}: Ensek Meterpoints Reference Glue Job completed successfully".format(
                        datetime.now().strftime("%H:%M:%S")
                    )
                )
