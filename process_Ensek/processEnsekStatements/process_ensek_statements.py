...
class Statements:
    ...
    def extract_data(self, data, account_id, k, dir_s3_key):
        ...
        filename = 'statements_' + str(account_id) + '.csv'
        df_extract_string = df__extract.to_csv(None, index=False)
        # print(df_extract_string)
        k.key = dir_s3_key['s3_key']['Statements'] + filename
    ...
    def log_error(self, error_msg, error_code=''):
        ...
        with open(logs_dir_path + 'statements' + time.strftime('%d%m%Y') + '.csv', mode='a') as errorlog:
            employee_writer = csv.writer(errorlog, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            employee_writer.writerow([error_msg, error_code])

    def processAccounts(self, account_ids, k, dir_s3):
        api_url, head = util.get_ensek_api_info1('statements')
