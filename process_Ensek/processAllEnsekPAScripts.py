...
def process_all_ensek_pa_scripts():
    ...
    try:
        ...
        subprocess.run([pythonAlias, "processEnsekMeterpoints/process_ensek_meterpoints_no_history.py"], check=True)
        print(
            "{0}: Meter Points completed in {1:.2f} seconds".format(
    ...
    try:
        ...
        start = timeit.default_timer()
        subprocess.run([pythonAlias, "processEnsekReadings/process_ensek_internal_readings.py"], check=True)
        print(
            "{0}: Internal Readings completed in {1:.2f} seconds".format(
                datetime.now().strftime("%H:%M:%S"), float(timeit.default_timer() - start)
            )
        )
        util.batch_logging_update(job_id, "e")
