...
class StartTransactionsJobs:
    ...
    def submit_transactions_job(self):
        ...
        try:
            ...
            subprocess.run([self.pythonAlias, "process_ensek_transactions.py"], check=True)
            print(
                "{0}: Process Ensek Transactions completed in {1:.2f} seconds".format(
    ...
    def submit_transactions_staging_gluejob(self):
        try:
            ...
            obj_stage = glue.ProcessGlueJob(
                job_name=jobName, s3_bucket=s3_bucket, environment=environment, processJob="transactions"
            )
            job_response = obj_stage.run_glue_job()
            if job_response:
                print(
                    "{0}: Transactions Staging Job Completed successfully".format(datetime.now().strftime("%H:%M:%S"))
                )
    ...
    def submit_transactions_gluejob(self):
        try:
            ...
            obj_submit_tarnsctaions_Gluejob = glue.ProcessGlueJob(
                job_name=jobname, s3_bucket=s3_bucket, environment=environment, processJob="transactions"
            )
