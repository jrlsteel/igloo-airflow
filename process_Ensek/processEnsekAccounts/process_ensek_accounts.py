...
iglog = common.utils.IglooLogger('Process_Ensek_Accounts')
class Accounts:
    max_calls = conf.config.api_config['max_api_calls']
    rate = conf.config.api_config['allowed_period_in_secs']
    ...
    def get_api_response(self, api_url, head, account_id, metrics):
        ...
        timeout = conf.config.api_config['connection_timeout']
        retry_in_secs = conf.config.api_config['retry_in_secs']
        i = 0
        attempt_num = 0
        total_api_time = 0.0
        api_call_start = 0.0
    ...
    def processAccounts(self, account_ids, k, dir_s3, metrics):
        ...
        api_url, head = common.utils.get_ensek_api_info1('accounts')
...
def main():
    ...
    if conf.config.test_config['enable_manual'] == 'Y':
        account_ids = conf.config.test_config['account_ids']

    if conf.config.test_config['enable_file'] == 'Y':
        account_ids = common.utils.get_Users_from_s3(s3)

    if conf.config.test_config['enable_db'] == 'Y':
        account_ids = common.utils.get_accountID_fromDB(False)

    if conf.config.test_config['enable_db_max'] == 'Y':
        ...
        try:
            for i in range(total_processes + 1):
                ...
                if i == total_processes:
                    t = multiprocessing.Process(target=p1.processAccounts, args=(account_ids[last_value:], s3_con(bucket_name), dir_s3, [metrics]))
                else:
                    t = multiprocessing.Process(target=p1.processAccounts, args=(account_ids[last_value:uv], s3_con(bucket_name), dir_s3, [metrics]))
                last_value = uv
