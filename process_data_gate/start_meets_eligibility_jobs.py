...
class MeetsEligibilityJobs:
    ...
    def submit_meets_eligibility_gluejob(self):
        try:
            ...
            # Batch Logging
            util.batch_logging_insert(self.meets_eligibility_jobid,
                                      53,
                                      'meets_eligibility_gluejob',
                                      'start_meets_eligibility_jobs.py')

            obj_meets_eligibility = glue.ProcessGlueJob(job_name=jobName, s3_bucket=s3_bucket,
                                                        environment=environment, processJob='meets_eli')
            meets_eligibility_job_response = obj_meets_eligibility.run_glue_job()

            if meets_eligibility_job_response:

                print("{0}: Meets Eligibility job completed successfully".format(
...
if __name__ == '__main__':
    ...
    util.batch_logging_insert(s.all_jobid, 53, 'all_meets_eligibility_jobs',
                              'start_meets_eligibility_jobs.py')
